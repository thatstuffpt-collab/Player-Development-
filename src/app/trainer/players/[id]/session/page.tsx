...
export default function RealPlayerSessionPage() {
  ...
  async function saveLog(event: FormEvent) {
    ...
    try {
      ...
      setResult(""); setNote(""); setSpot(""); setCustomDrill("");
    } catch (err) {
        ...
        <div className={styles.stack}>
          <section className={styles.panel}>
            ...
            <form className={styles.quickForm} onSubmit={saveLog}>
              ...
              {shootingSelected && <label>Court spot<select value={spot} onChange={(e) => setSpot(e.target.value)}><option value="">Choose spot</option>{shootingSpots.map((item) => <option key={item}>{item}</option>)}</select></label>}
              {!shootingSelected && <label>Spot / side (optional)<input value={spot} onChange={(e) => setSpot(e.target.value)} placeholder="Right hand, left side..." /></label>}
              <label>Result<input value={result} onChange={(e) => setResult(e.target.value)} placeholder="7/10, 18.4 sec, 12 reps" /></label>
              <label>What does it mean next?<select value={next} onChange={(e) => setNext(e.target.value)}><option>Keep progressing</option><option>Revisit next session</option><option>Goal met</option><option>Change focus</option></select></label>
              <label className={styles.full}>Coach note<textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} /></label>
              <div className={styles.actions}><button type="button">🎙 Voice-to-text</button><button className={styles.primary} disabled={busy}>Save Quick Log</button></div>
            </form>
          </section>
