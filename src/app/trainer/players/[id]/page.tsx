...
export default function PlayerProfilePage() {
  ...
  return <main className="players-shell">
    ...
    <div className="profile-actions"><Link className="primary-link-button" href={`/trainer/players/${player.id}/session`}>Start Session</Link><button className="ghost-button" onClick={()=>setEditing(v=>!v)}>{editing?"Close edit":"Edit profile"}</button>{player.evaluations.length?<Link className="primary-link-button" href={`/trainer/evaluation/reevaluate?playerId=${player.id}`}>Reevaluate</Link>:<Link className="primary-link-button" href={`/trainer/evaluation/new?playerId=${player.id}`}>Evaluate</Link>}</div></header>
    ...
    <section className="session-card"><div className="section-heading"><div><span className="section-kicker">DEVELOPMENT TIMELINE</span><h2>Development history</h2></div></div>
      {[...player.evaluations.map(x=>({date:x.evaluatedAt,title:"Formal Evaluation",detail:x.shortTermGoal||x.priorityAreas.join(" · ")})),...player.progressEvents.map(x=>({date:x.occurredAt,title:x.title,detail:x.result})),...player.athleticTests.map(x=>({date:x.testedAt,title:x.testName,detail:`${x.value} ${x.unit}`})),...player.achievements.map(x=>({date:x.achievedAt,title:"Achievement",detail:x.title}))].sort((a,b)=>new Date(b.date).getTime()-new Date(a.date).getTime()).slice(0,30).map((item,index)=><div className="profile-list-row" key={`${item.date}-${item.title}-${index}`}><div><strong>{item.title}</strong><p className="support-copy">{item.detail}</p></div><span>{new Date(item.date).toLocaleDateString()}</span></div>)}
      {!player.evaluations.length&&!player.progressEvents.length&&!player.athleticTests.length&&!player.achievements.length&&<p className="support-copy">Development history will appear here as evaluations, results, tests and achievements are saved.</p>}
    </section>
