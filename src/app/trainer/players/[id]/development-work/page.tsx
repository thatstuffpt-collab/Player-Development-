...
export default function DevelopmentWorkPage(){
 ...
 useEffect(()=>{let cancelled=false;authenticatedFetch(`/api/trainer/players/${params.id}`).then(async r=>{const p=await r.json();if(!r.ok)throw new Error(p.error??"Could not load player.");if(!cancelled)setPlayer(p.player)}).catch(e=>{if(!cancelled)setError(e instanceof Error?e.message:"Could not load player.")});return()=>{cancelled=true}},[params.id]);
